...
class UpNextState(object):  # pylint: disable=too-many-public-methods
    ...
    def _set_detect_time(self):
        # IntroDB provides an authoritative outro timestamp.
        # No additional credit detection is needed.
        if self.introdb_detected:
    ...
    def set_detected_popup_time(self, detected_time):
        if getattr(self, 'introdb_detected', False):
            self.log(
                'Popup: ignoring detector timestamp {0:.2f}s; '
                'IntroDB timestamp is {1:.2f}s'.format(
                    detected_time,
                    self.popup_time
                ),
                utils.LOGINFO
    ...
    def set_popup_time(self, total_time):
        ...
        # Use 1s offset from total_time to try and avoid race condition
        # with internal Kodi playlist handling
        ...
        try:
            ...
            introdb_time = None
            self.log(
                'IntroDB lookup error: {0}'.format(e),
                utils.LOGERROR
            )

        if introdb_time is not None and introdb_time > 0:
            popup_time = float(introdb_time)
            self.introdb_detected = True
            self.chapter_detected = False
            self.popup_cue = SETTINGS.sim_cue == constants.SETTING_ON

            self.log(
                'Popup: using IntroDB outro at {0:.2f}s'.format(
                    popup_time
                ),
                utils.LOGINFO
            )
        ...
        # Chapter detection
        # ============================================================
        if not popup_time and SETTINGS.detect_chapters:
            threshold = SETTINGS.detect_chapters_threshold

            self.log(
                'Attempting chapter detection with {0}% threshold'.format(
                    threshold
                )
            )
            ...
            if chapter_time:
                ...
                self.log(
                    'Popup: using chapter detection at {0}s'.format(
                        popup_time
                    ),
                    utils.LOGINFO
                )
                ...
                self.log(
                    'Chapter detection: no suitable chapters found'
                )
        ...
        if not popup_time and self.get_plugin_type():
            popup_duration = utils.get_int(
                self.data,
                'notification_time',
                0
            )

            popup_time = utils.get_int(
                self.data,
                'notification_offset',
                0
            )
            ...
            if (
                0 < popup_time
                <= total_time - constants.POPUP_MIN_DURATION
            ):
                self.popup_cue = (
                    SETTINGS.sim_cue != constants.SETTING_OFF
                )
            else:
                popup_time = 0
        ...
        # ============================================================
        if not popup_time:
            popup_duration = SETTINGS.popup_durations[
                max(
                    0,
                    0,
                    *[
                        duration
                        for duration in SETTINGS.popup_durations
            ...
            self.popup_cue = (
                SETTINGS.sim_cue == constants.SETTING_ON
            )
        ...
        popup_pct = (
            self.popup_time / self.total_time * 100
            if self.total_time > 0
            else 0
        )

        self.log(
            'Popup: due at {0}s of {1}s ({2:.1f}%) '
            '(auto_play: {3})'.format(
                self.popup_time,
                self.total_time,
                popup_pct,
